...
**完整的证明数据**（两轮哈希、时间戳、全部 20 个区块高度、区块哈希、公钥）被嵌入链上 Witness JSON 中。Indexer 使用自己的 Bitcoin Core RPC 独立重算两轮哈希，如果重算结果不匹配则拒绝该铸造。在没有真实原始区块数据访问能力的情况下，伪造有效证明在计算上是不可行的。
...
| API 中继（无全节点） | 证明需要 20 个随机区块的原始数据。本地 ~1s vs 远程 ~5-15s。 |
