...
[4] Commit交易   BTC → 包含铭文脚本树的Taproot地址
         │
[5] Reveal交易   脚本路径花费 → 铭文 + OP_RETURN + 铸造费
         │
[6] 确认入块     区块打包 → Indexer验证7条规则 → 记入500 NXS
```
...
2. **OP\_RETURN**：以 `NXS:` 开头，正确的ASCII格式（`NXS:1:w=<16hex>:p=<16hex>`）
...
| 发现 | 状态 |
|------|------|
| 身份未绑定签名密钥 | ✅ 已修复 — 新增 `pk` 字段，Indexer验证 |
| JSON序列化不确定 | ✅ 不适用 — Rust `serde_json` 按struct字段顺序确定性序列化 |
| proof去重竞态条件 | ✅ 不适用 — 单线程顺序处理 |
| 昂贵proof验证被DoS | ✅ 已修复 — 轻量预过滤在完整验证之前 |
| 多Indexer状态分歧 | ⚠️ 已知限制 — 与BRC-20/Runes相同（链下Indexer模型） |
